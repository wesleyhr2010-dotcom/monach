...
## Objetivo
Padronizar o envio de e-mails do NEXT-MONARCA em um único provedor (Brevo), cobrindo tanto os e-mails de Supabase Auth (reset/convite) quanto os transacionais da aplicação (aprovação de documento, confirmação de acerto, candidatura etc.).
...
- Nenhum dado sensível em plaintext no corpo.

## Edge cases
- Limite diário atingido → fila local + retry no dia seguinte (ou upgrade).
- Falha SMTP → Supabase reemite; transacional loga e alerta.
- Domínio sem DKIM válido → e-mails caem em spam; sinalizar no onboarding de DNS.
- Template deletado acidentalmente → Server Action falha → log + fallback manual.

## Dependências
- `SPEC_ENVIRONMENT_VARIABLES.md` — `BREVO_API_KEY`, `BREVO_SMTP_USER/PASS`.
- `SPEC_LOGIN.md` / `SPEC_RECUPERAR_CONTRASENA.md` — fluxos que disparam reset.
- `SPEC_ADMIN_LEADS.md` — e-mail de aprovação/rejeição.
- `SPEC_ADMIN_EQUIPE.md` — convites de consultora/revendedora.
