# SPEC — Design System (Módulos da App Revendedoras)

## Objetivo
Consolidar tokens, átomos, moléculas, organismos e templates que compõem o PWA mobile-first da revendedora, garantindo consistência visual baseada no manual de identidade Monarca e na filosofia minimalista-elegante.

## Atores
- **Revendedora** — consumidora final do PWA.
- **Design (Uma)** — mantém tokens e compõe novos módulos.
- **Dev (Dex)** — implementa componentes seguindo exatamente os tokens e classes aqui definidos.

## Fluxo
1. Cada tela parte dos tokens de cor, tipografia, espaçamento e raio descritos na seção 0.
2. Composições começam nos átomos (Avatar, RankBadge, StatusBadge, etc.) e crescem até templates (DashboardLayout).
3. Antes de marcar tela como pronta, aplicar o checklist de conformidade.
4. Estados assíncronos usam SkeletonCard; estados vazios usam EmptyState.

## Regras de negócio
- Tema light/cream é independente do dark mode do admin.
- Apenas fontes Playfair Display e Raleway são permitidas.
- Cards usam cor sólida `#EBEBEB` sem borda nem gradiente; sombras apenas sutis na bottom nav.
- Pills, badges e avatares sempre com `border-radius: 100px`; mínimo `12px` em qualquer container.
- Ícones Lucide outline, stroke 1.5px.
- Cor primary `#35605A` reservada para elementos ativos e CTAs.
- Contraste mínimo 4.5:1 para texto sobre background.

## Edge cases
- Tela em iPhone com notch → respeitar `env(safe-area-inset-top/bottom)`.
- Avatar sem foto → fallback com iniciais sobre `--app-surface`.
- Valor monetário longo em StatCard → usar Playfair Bold, truncar ou escalar se necessário, nunca quebrar hierarquia visual.
- Badges de nível com rótulo muito longo → manter pill; não uppercase.

## Dependências
- `SPEC_FRONTEND.md` — layouts e rotas que consomem estes módulos.
- `SPEC_SKELETON_EMPTY_STATES.md` — padrão de skeleton/empty.
- `SPEC_LOGIN.md`, `SPEC_HOME.md`, `SPEC_MALETA.md`, `SPEC_PROGRESSO.md` e demais SPECs da revendedora.
- Manual de Identidade Visual Monarca (`docs/assets/`).
