# SPEC — Segurança de Endpoints (Rate Limiting, Validação, Upload Seguro)

## Objetivo
Proteger endpoints públicos e semi-públicos contra abuso, entradas maliciosas e uploads inválidos, usando rate limiting (Upstash Redis), validação Zod estrita e checagem de magic bytes.

## Atores
- **Route Handlers** (`/api/*`) — aplicam rate limit e validação.
- **Server Actions** — usam Zod por action e checam sessão.
- **Upstash Redis** — armazena contadores de rate limit (sliding/fixed window).
- **next.config.ts** — injeta security headers globais.

## Fluxo
1. Request chega em Route Handler → extrai identificador (userId ou IP).
2. `rateLimiters.<scope>.limit(key)` consulta Redis; se estourou → 429 com `Retry-After`.
3. Body é validado via `safeParse` do Zod.
4. Em upload, `validateMagicBytes` lê os primeiros 8 bytes e confere assinatura.
5. Security headers aplicados globalmente (CSP, X-Frame, Referrer-Policy).

## Regras de negócio
- `POST /api/upload-r2`: 10 req/min/user; auth obrigatória; tipo + tamanho + magic bytes.
- `POST /api/track-evento`: 100 req/min/IP; público; Zod estrito; CORS só para domínio próprio.
- `POST /app/login`: 5 tentativas/15min (Supabase controla).
- `POST /app/login/recuperar`: 3 req/15min.
- Nunca confiar em `Content-Type` do cliente — sempre magic bytes.
- Sem `UPSTASH_*` em dev → rate limit graceful fallback (permite); em prod é obrigatório.

## Edge cases
- IP vazio (`x-forwarded-for` ausente) → usar `'unknown'` como chave (pior caso: bucket compartilhado).
- Upload com tipo válido mas magic bytes divergentes → 400 sem consumir quota R2.
- Redis offline temporário → retornar 503 ou permitir pass-through? Decisão: pass-through + log warning.
- CSP bloqueia OneSignal → incluir `https://cdn.onesignal.com` em `script-src` e `connect-src`.

## Dependências
- `SPEC_API_UPLOAD_R2.md` — rota de upload protegida por este SPEC.
- `SPEC_SECURITY_RBAC.md` — roles permitidas por action.
- `SPEC_SECURITY_DATA_PROTECTION.md` — criptografia e PII.
- `SPEC_ENVIRONMENT_VARIABLES.md` — `UPSTASH_*`.
- `SPEC_ERROR_HANDLING.md` — códigos `RATE_LIMITED`, `VALIDATION_ERROR`.
