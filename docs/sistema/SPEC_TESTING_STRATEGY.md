...
## Objetivo
Definir stack (Vitest, Playwright, MSW), cobertura mínima e fluxos críticos que precisam de teste automatizado para garantir qualidade contínua do sistema.

## Atores
- **Dev (Dex)** — escreve testes unitários/integração junto com cada feature.
- **QA (Quinn)** — valida cobertura e adiciona E2E para os 5 fluxos principais.
- **CI (GitHub Actions)** — executa `test`, `test:coverage`, `test:e2e` e bloqueia PR com verde incompleto.
- **Banco de teste isolado** (Supabase local) — suporta testes de integração.

## Fluxo
1. Feature nova → dev adiciona unit tests para funções puras (>=90% em cálculos críticos).
2. Server Actions críticas → integration test com banco real.
3. Fluxos de ponta (login, venda, admin maleta, brinde, devolução) → E2E Playwright.
4. PR abre → CI roda stack completa; coverage abaixo do limite bloqueia merge.
5. Mocks de Supabase Auth/OneSignal/Brevo feitos com MSW quando unit test exige.

## Regras de negócio
- Coverage mínimo: cálculos 90%, Server Actions 80%, componentes 40%, E2E 5 fluxos.
- `createMaleta` precisa cobrir lock de estoque concorrente.
- `closeMaleta` precisa congelar snapshot de valores (teste imutabilidade).
- `awardPoints` precisa validar `tipo=unico`, limite diário e saldo insuficiente.
- Testes nunca usam produção/staging — apenas Supabase local ou banco de teste dedicado.

## Edge cases
- Supabase local indisponível → CI falha rápido com mensagem clara.
- Teste E2E flaky por câmera/permissions → usar `context.grantPermissions(['camera'])`.
- Seed acidentalmente entra em banco de teste duplicado → usar `reset` entre suites.
- Cobertura cai ao remover feature → atualizar metas no `vitest.config.ts` apenas com revisão.

## Dependências
- `SPEC_DEPLOY_STRATEGY.md` — pipeline CI.
- `SPEC_BACKEND.md` — actions testadas.
- `SPEC_MIGRATIONS_SEED.md` — seed para banco de teste.
- `SPEC_SECURITY_API_ENDPOINTS.md` — schemas Zod testáveis.
