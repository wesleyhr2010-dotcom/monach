# SPEC — Proteção de Dados Sensíveis (PII, Documentos, Bancários)

## Objetivo
Garantir que PII e dados financeiros (CI/RUC, conta, alias Bancard, documentos, endereço) sejam armazenados cifrados, acessados via signed URLs com auditoria e nunca vazem em logs ou na vitrina pública.

## Atores
- **Prisma Middleware** — cifra/decifra campos sensíveis de `DadosBancarios` em AES-256-GCM.
- **API de Upload** — grava documentos pessoais em `private/` no R2.
- **Server Actions** — geram signed URLs sob demanda (expira em 1h) com log de auditoria.
- **`sanitizeForLog`** — remove campos sensíveis antes de `console.error`.

## Fluxo
1. Form de dados bancários → Action salva → middleware cifra em `iv:encrypted:authTag`.
2. Leitura da conta → middleware decifra antes de devolver ao caller.
3. Upload de documento pessoal → path `private/resellers/{id}/docs/...` no R2.
4. Revendedora/admin pede visualização → Action valida ownership e emite signed URL (1h).
5. Todo acesso a documento é logado (`event: document_accessed`) sem PII.

## Regras de negócio
- Campos cifrados: `alias_ci_ruc`, `alias_valor`, `cuenta`, `ci_ruc`.
- `ENCRYPTION_KEY` = 32 bytes hex; rotação exige re-encrypt dos registros.
- Documentos nunca têm URL pública permanente.
- COLABORADORA só acessa documentos de revendedoras sob `manager_id` próprio.
- Vitrina pública expõe apenas `name`, `avatar_url`, `slug`, link wa.me — nunca email, endereço, taxa, role.
- UI mascara conta bancária e alias ao exibir.
- Retenção: dados bancários removidos após 1 ano de inatividade (processo manual).

## Edge cases
- Registro salvo sem middleware (seed direto) → não decifra; detectar padrão `iv:enc:tag`.
- `ENCRYPTION_KEY` rotacionada sem migração → dados antigos ficam ilegíveis.
- Signed URL vazada → mitigação via TTL curto (1h).
- Admin exclui revendedora → apenas `ativo=false`; deleção completa é manual (direito de exclusão LGPD).

## Dependências
- `SPEC_ENVIRONMENT_VARIABLES.md` — `ENCRYPTION_KEY`.
- `SPEC_API_UPLOAD_R2.md` — paths privados.
- `SPEC_SECURITY_RBAC.md` — ownership por role.
- `SPEC_LOGGING_MONITORING.md` — sanitização de PII em logs.
- `SPEC_ADMIN_DOCUMENTOS_ACERTOS.md` — UI admin de documentos.
