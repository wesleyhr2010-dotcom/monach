# SPEC — Admin: Analytics, Notificações Push e Alertas

## Objetivo
Entregar ao admin um painel operacional com KPIs e gráficos de gestão de maletas, configuração de templates de push OneSignal e um widget persistente de alertas (maletas vencendo, documentos pendentes, leads novos).

## Atores
- **SUPER_ADMIN** — vê tudo consolidado e configura OneSignal.
- **CONSULTORA** — vê analytics do seu grupo (`getResellerScope`).
- **OneSignal** — entrega dos pushes.
- **Cron diário** — alimenta métricas e dispara alertas (`SPEC_CRON_JOBS.md`).

## Fluxo (Analytics)
1. Admin acessa `/admin/analytics`.
2. Sistema calcula KPIs do período (maletas ativas, devolvidas, taxa de atraso, ticket médio, revendedoras com maleta, tempo médio).
3. Renderiza gráficos (fluxo de maletas, donut de status) e tabela top 10 revendedoras.
4. Admin pode trocar período (7d / 30d / 3m / 12m).

## Fluxo (Configuração OneSignal)
1. SUPER_ADMIN acessa `/admin/configuracoes/notificacoes`.
2. Edita templates de push (título, corpo, CTA) por tipo de evento.
3. Testa envio para um device específico antes de salvar.
4. Ativa/desativa categorias globais.

## Fluxo (Widget de Alertas)
1. Presente em todas as telas admin (sininho no topbar).
2. Agrupa: maletas vencendo em ≤ 2 dias, documentos pendentes, solicitações de brindes, leads novos.
3. Clique leva à tela correspondente.

## Regras de negócio
- CONSULTORA só vê dados do seu grupo; SUPER_ADMIN vê tudo.
- KPIs são calculados no período selecionado (absoluto ou relativo).
- Templates OneSignal seguem os tipos definidos em `SPEC_NOTIFICACOES.md`.
- Widget atualiza-se sob demanda (revalidar a cada navegação).
- Taxa de atraso = maletas `atrasada` / total no período.

## Edge cases
- Consultora sem dados no período → gráficos vazios, cards zerados.
- Template OneSignal vazio → bloqueia salvar.
- Falha no OneSignal → logar + exibir erro no painel.
- Widget com zero alertas → sininho sem badge.
- Mudança de template durante envio em massa → aplica só aos novos.

## Dependências
- `SPEC_ADMIN_LAYOUT.md` — shell e sininho.
- `SPEC_NOTIFICACOES.md` — tipos de evento.
- `SPEC_CRON_JOBS.md` — dados agregados.
- `SPEC_LOGGING_MONITORING.md` — registros de envio.
