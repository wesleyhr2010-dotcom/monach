# SPEC — Admin: Configurações (Commission Tiers, Contratos PDF)

## Objetivo
Permitir ao admin configurar a tabela de faixas de comissão por faturamento mensal da revendedora e manter os modelos de contrato PDF usados no onboarding.

## Atores
- **SUPER_ADMIN** — único com acesso de escrita.
- **Sistema de comissão** — consome `CommissionTier` no cálculo mensal.
- **Revendedora** — visualiza seu tier atual na home.

## Fluxo (Commission Tiers)
1. Admin acessa `/admin/commission-tiers`.
2. Adiciona/edita faixas (label, `min_sales_value`, `commission_pct`).
3. Ao salvar, novos cálculos usam a tabela atualizada.

## Fluxo (Contratos)
1. Admin acessa `/admin/contratos` e faz upload de PDFs (contrato padrão, adendos etc.).
2. Cada contrato fica disponível para vincular a revendedoras.
3. Revendedora vê e aceita o contrato no perfil/onboarding.

## Regras de negócio
- Sempre deve existir pelo menos uma faixa com `min_sales_value = 0` (faixa base).
- Não é permitido deletar a faixa base.
- Não pode haver duas faixas com mesmo `min_sales_value`.
- Revendedora enquadra-se sempre na **maior** faixa cujo mínimo seu faturamento supera.
- Mudanças em tiers **não afetam** maletas já `concluida` (valores congelados).
- Período de referência: mês civil em `America/Asuncion`.

## Edge cases
- Faturamento exatamente igual ao `min_sales_value` → inclui-se na faixa.
- Tabela com só uma faixa base → todas revendedoras caem nela.
- Contrato PDF substituído → histórico mantém versão anterior vinculada aos aceites.
- Faixa editada durante o mês → vale para fechamento do mês corrente (informativo).

## Dependências
- `SPEC_HOME.md` — consome tier atual.
- `SPEC_ADMIN_MALETAS.md` — cálculo da comissão no fechamento.
- `SPEC_DATABASE.md` — `CommissionTier`, `Contrato`.
- `SPEC_ADMIN_DOCUMENTOS_ACERTOS.md` — aceite de contratos.
