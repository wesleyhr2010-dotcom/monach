...
## Objetivo
Permitir ao admin/consultora conferir fisicamente os itens devolvidos pela revendedora, registrar as quantidades recebidas, resolver divergências e fechar a maleta com o cálculo final de comissão e baixa de estoque.

## Atores
- **SUPER_ADMIN / CONSULTORA** — executa a conferência.
- **Revendedora** — já submeteu o acerto e aguarda.
- **Sistema de estoque** — recebe movimento `devolucao_maleta`.
- **Sistema de comissão** — calcula valores finais da revendedora e da consultora.

## Fluxo
1. Admin acessa `/admin/maletas/[id]/conferir` (maleta em `aguardando_revisao`).
2. Vê comprovante enviado pela revendedora + tabela de itens (enviado, vendido, recibido, diferença).
3. Digita quantidade recebida por item; sistema calcula diferenças em tempo real.
4. Opcionalmente adiciona nota do acerto (justificando divergências).
5. Confirma → maleta `concluida`, estoque ajustado, comissão calculada, notificação `acerto_confirmado` enviada.

## Regras de negócio
- Só maletas em `aguardando_revisao` podem ser conferidas.
- Comprovante de devolução (`comprovante_devolucao_url`) é pré-condição obrigatória.
- Diferenças negativas (item faltante) são destacadas com ⚠️.
- Nota do acerto registrada em campo histórico da maleta.
- Cálculo final: comissão revendedora = `faturamento × taxa_comissao`; comissão consultora incide sobre o grupo.
- Após confirmação, maleta é **imutável**.
- Estoque das peças devolvidas retorna a `stock_quantity` via `EstoqueMovimento(tipo='devolucao_maleta')`.

## Edge cases
- Divergência grande → admin pode ajustar e registrar nota; bloqueia só se quiser.
- Revendedora tentando reabrir após conferida → bloqueado.
- Falha no registro de estoque → transação é revertida.
- Conferência parcial salva como rascunho (se suportado) → não muda status.
- Maleta sem comprovante enviado → bloqueia conferência.

## Dependências
- `SPEC_ADMIN_MALETAS.md` — contexto do fluxo.
- `SPEC_DEVOLUCAO.md` — origem do estado `aguardando_revisao`.
- `SPEC_ADMIN_PRODUTOS.md` — movimentação de estoque.
- `SPEC_NOTIFICACOES.md` — evento `acerto_confirmado`.
- `SPEC_DATABASE.md` — `Maleta`, `MaletaItem`, `EstoqueMovimento`.
