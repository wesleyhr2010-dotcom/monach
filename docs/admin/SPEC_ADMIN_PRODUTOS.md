...
## Objetivo
Permitir ao SUPER_ADMIN cadastrar e manter o catálogo de produtos (com variantes e imagens), controlar estoque (com histórico de movimentações) e organizar produtos por categorias hierárquicas.

## Atores
- **SUPER_ADMIN** — único com acesso de escrita a produtos e categorias.
- **Sistema de maletas** — consome `stock_quantity` ao reservar/devolver itens.
- **Cloudflare R2** — armazena imagens.

## Fluxo
1. Admin acessa `/admin/produtos` e vê lista com alertas de estoque baixo.
2. "Novo Produto" → preenche SKU, nome, descrição, tipo (simple/variable), upload de imagens.
3. Se `variable`, adiciona variantes (cor, tamanho) com preços e estoque individuais.
4. Categorias são associadas via relação `ProductCategory`.
5. Ajustes manuais de estoque são registrados em `EstoqueMovimento` com motivo.
6. Em `/admin/categorias`, mantém árvore hierárquica (parent/children).

## Regras de negócio
- Produtos `simple` usam `Product.price`; `variable` usam `ProductVariant.price`.
- SKU é único; `slug` da categoria é único.
- Estoque é decrementado na reserva da maleta (`tipo = 'reserva_maleta'`) e devolvido em `devolucao_maleta`.
- Toda movimentação de estoque gera registro em `EstoqueMovimento` com `admin_id`.
- Categoria pode ter subcategorias (`parent_id` opcional).
- Produto desativado não aparece em listas públicas/catálogo de maletas novas.

## Edge cases
- Produto em maleta ativa sendo desativado → mantém na maleta, bloqueia uso em novas.
- Estoque negativo por ajuste errado → permitir mas alertar visualmente.
- Variante sem preço → usa fallback do produto pai.
- Deletar categoria com produtos → bloqueia, exige reatribuição.
- Upload > 5 MB → bloqueia com mensagem.

## Dependências
- `SPEC_ADMIN_MALETAS.md` — consome produtos.
- `SPEC_API_UPLOAD_R2.md` — upload de imagens.
- `SPEC_DATABASE.md` — `Product`, `ProductVariant`, `Category`, `ProductCategory`, `EstoqueMovimento`.
- `SPEC_VITRINE_PUBLICA.md` — exibe produtos via `product_variant`.
