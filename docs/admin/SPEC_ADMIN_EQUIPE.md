# SPEC — Admin: Gestão de Equipe (Consultoras e Revendedoras)

## Objetivo
Oferecer ao admin CRUD de consultoras (SUPER_ADMIN) e de revendedoras (SUPER_ADMIN e CONSULTORA no seu grupo), com perfis completos, métricas e vínculo consultora↔revendedoras.

## Atores
- **SUPER_ADMIN** — gerencia consultoras e todas as revendedoras.
- **CONSULTORA** — vê e gerencia apenas suas revendedoras.
- **Sistema de e-mails** — envia convites (definição de senha).
- **Supabase Auth** — cria usuário no ato do cadastro.

## Fluxo (Consultoras)
1. SUPER_ADMIN acessa `/admin/consultoras` e vê lista com desempenho.
2. "Nova Consultora" → preenche dados + taxa de comissão → sistema cria user Supabase com role `CONSULTORA`.
3. Consultora recebe e-mail com link de definição de senha.
4. Perfil `/admin/consultoras/[id]` mostra revendedoras vinculadas, faturamento, comissão.

## Fluxo (Revendedoras)
1. Admin acessa `/admin/revendedoras` (SUPER_ADMIN vê todas; CONSULTORA vê do seu grupo).
2. "Nova Revendedora" → preenche dados, vincula consultora, define `taxa_comissao`.
3. Sistema cria user Supabase com role `REVENDEDORA` + envia convite.
4. Perfil `/admin/revendedoras/[id]` mostra dados, maletas, documentos, pontos, vitrine.

## Regras de negócio
- Apenas SUPER_ADMIN cria/edita consultoras.
- Consultora só vê suas revendedoras; SUPER_ADMIN pode reassinalar.
- `taxa_comissao` é definida pelo admin e é read-only para revendedora.
- Desativação (`ativo = false`) preserva histórico mas bloqueia login.
- Taxa da consultora incide sobre faturamento das suas revendedoras (para relatório).

## Edge cases
- E-mail já em uso → erro no cadastro.
- Revendedora sem consultora → permitido (pode ser vinculada depois).
- Consultora com revendedoras ativas sendo desativada → exige reatribuição.
- Atualização de taxa durante maleta em andamento → aplica só em novas maletas.

## Dependências
- `SPEC_ADMIN_LAYOUT.md` — RBAC.
- `SPEC_ADMIN_DOCUMENTOS_ACERTOS.md` — documentos da revendedora.
- `SPEC_ADMIN_CONSULTORA_PERFIL.md` — visão detalhada da consultora.
- `SPEC_EMAILS.md` — templates de convite.
- `SPEC_DATABASE.md` — `Reseller`, consultora (também `Reseller` com role).
