...
- [~] **Centro de notificações** no PWA + campanhas push no admin. Ref.: [`revendedoras/SPEC_NOTIFICACOES.md`](./revendedoras/SPEC_NOTIFICACOES.md), [`admin/SPEC_ADMIN_ANALYTICS_NOTIFICATIONS.md`](./admin/SPEC_ADMIN_ANALYTICS_NOTIFICATIONS.md), [`prd/PRD_OneSignal_PWA.md`](./prd/PRD_OneSignal_PWA.md).
  ...
  - [ ] **Histórico persistente de notificações no PWA** — tela `/app/notificaciones` com lista agrupada por día (Hoy, Ayer, Anteriores). **NÃO incluído na entrega atual de preferências push.** Requer criação da tabela `Notificacao` no Prisma + instrumentação de todas las ações geradoras (nova maleta, prazo, acerto, brinde, pontos, documento). Ref.: [`revendedoras/SPEC_NOTIFICACOES.md`](./revendedoras/SPEC_NOTIFICACOES.md).
  - [ ] **Admin AlertBell** — sininho persistente no header admin com badge de maletas `aguardando_revisao` + drawer de devoluções pendentes.
