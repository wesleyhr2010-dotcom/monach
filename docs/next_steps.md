...
- [~] **Gestão de Equipe** no admin (revendedoras + consultoras, vínculos `manager_id`). Ref.: [`admin/SPEC_ADMIN_EQUIPE.md`](./admin/SPEC_ADMIN_EQUIPE.md), [`admin/SPEC_ADMIN_CONSULTORA_PERFIL.md`](./admin/SPEC_ADMIN_CONSULTORA_PERFIL.md).
  ...
  - [ ] **Criação via Supabase Auth + convite por email** — `createConsultora` e `criarRevendedora` ainda criam apenas no banco local; falta integrar `supabase.auth.admin.createUser` + envio de email de definição de senha via Brevo.
  - [ ] **Sidebar da Consultora** com escopo restrito e `/admin/minha-conta` (perfil + extrato de comissões).
  - [ ] **Analytics do Grupo** filtrado para consultora (`/admin/analytics`).
...
- [ ] **Configurar SMTP do Brevo no Supabase Dashboard** — adicionar credenciais SMTP do Brevo (host `smtp-relay.brevo.com`, port `587`, user e senha SMTP gerados no painel Brevo) em Supabase Dashboard → Authentication → Settings → SMTP Settings, para que reset de senha, convite por email e magic links do Supabase Auth sejam enviados via Brevo (e não pelo SMTP padrão limitado). Os emails transacionais da aplicação (boas-vindas, confirmação de canje, etc.) já usam Brevo via SDK em `src/lib/emails.ts`; este item cobre apenas os emails do próprio Supabase Auth. Ref.: [`sistema/SPEC_EMAILS.md`](./sistema/SPEC_EMAILS.md) §10.
