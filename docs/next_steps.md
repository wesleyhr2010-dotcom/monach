...
- [~] **Centro de notificações** no PWA + campanhas push no admin. Ref.: [`revendedoras/SPEC_NOTIFICACOES.md`](./revendedoras/SPEC_NOTIFICACOES.md), [`admin/SPEC_ADMIN_ANALYTICS_NOTIFICATIONS.md`](./admin/SPEC_ADMIN_ANALYTICS_NOTIFICATIONS.md), [`prd/PRD_OneSignal_PWA.md`](./prd/PRD_OneSignal_PWA.md).
  ...
  - [x] **Configuração de notificações push no admin** (`/admin/config/notif-push`) — templates OneSignal (7 templates seedados: prazo D-3/D-1, maleta atrasada, devolução, nova maleta, brinde, pontos), tabela com toggle ativo/inativo e modal de edição, botão de teste de push para o admin logado, histórico de envios com status. Models `NotificacaoTemplate` e `NotificacaoLog` adicionados ao Prisma schema + sincronizados no banco. Ref.: [`admin/SPEC_ADMIN_ANALYTICS_NOTIFICATIONS.md`](./admin/SPEC_ADMIN_ANALYTICS_NOTIFICATIONS.md) §2, [`prd/PRD_OneSignal_PWA.md`](./prd/PRD_OneSignal_PWA.md).
