...
- [ ] **Atualizar Site URL do Supabase Auth ao migrar domínio** — o Site URL e Redirect URLs no Supabase Dashboard (`Authentication → URL Configuration`) devem ser atualizados ao mudar de preview (`*.vercel.app`) para produção (`monarcasemijoyas.com.py`). Senão, os links de reset de senha e magic links continuam apontando para o domínio antigo. ⚠️ **Ação manual obrigatória no dashboard.** Também adicionar `/app/nueva-contrasena` e `/admin/login/reset-password` à lista de Redirect URLs, e revisar o template "Reset Password" em `Authentication → Email Templates` para usar `{{ .ConfirmationURL }}` (não `{{ .SiteURL }}`). Enquanto isso, o fallback `src/app/auth/callback/route.ts` (2026-04-23) intercepta links que chegam na raiz com `?code=` e roteia para a página correta conforme o role.
