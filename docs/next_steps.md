...
- [~] **Configurar SMTP do Brevo no Supabase Dashboard** — fluxo de reset de senha do admin agora tem rota de callback (`/admin/login/reset-password`). Falta configuração manual no dashboard:
  - [x] Rota `/admin/login/reset-password` criada — página de definição de nova senha com tema dark do admin, consistente com `/app/nueva-contrasena`.
  - [ ] **Configuração manual no Supabase Dashboard** — adicionar credenciais SMTP do Brevo (host `smtp-relay.brevo.com`, port `587`, user e senha SMTP gerados no painel Brevo) em Supabase Dashboard → Authentication → Settings → SMTP Settings, para que reset de senha, convite por email e magic links do Supabase Auth sejam enviados via Brevo. Ref.: [`sistema/SPEC_EMAILS.md`](./sistema/SPEC_EMAILS.md) §10.
