...
- [ ] **Atualizar Site URL do Supabase Auth ao migrar domínio** — o Site URL e Redirect URLs no Supabase Dashboard (`Authentication → URL Configuration`) devem ser atualizados ao mudar de preview (`*.vercel.app`) para produção (`monarcasemijoyas.com.py`). Senão, os links de reset de senha e magic links continuam apontando para o domínio antigo. ⚠️ **Ação manual obrigatória no dashboard.** Adicionar também `https://<dominio>/auth/callback` à lista de Redirect URLs. O template "Reset Password" em `Authentication → Email Templates` já foi migrado para o fluxo OTP: usa `{{ .SiteURL }}/auth/callback?token_hash={{ .TokenHash }}&type=recovery` (sem depender de PKCE — funciona cross-device). Fluxo de recovery verificado em produção em 2026-04-23 via `src/app/auth/callback/route.ts` (aceita `token_hash` e `code`) + middleware que intercepta `?token_hash=`/`?code=` na raiz.
