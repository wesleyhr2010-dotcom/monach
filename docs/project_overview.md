...
| Dark Mode & Temas (Token Foundation) | **Concluído** | Phase 19 (2026-05-15): `@custom-variant dark` configurado em `globals.css`; tokens `--color-app-*` com variantes dark sob `.app-shell[data-theme="dark"]`; tokens `--admin-*` com light `:root` defaults e dark sob `.admin-layout[data-theme="dark"]` em `admin.css`; shadcn/ui overrides centralizados. Admin mantém dark via `data-theme="dark"` hardcoded (temporário até Phase 21 ThemeProvider). Build e lint passam. Ref.: `ROADMAP.md` Phase 19. |
