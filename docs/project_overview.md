...
| Vitrina pública `/vitrina/[slug]` | **Stub criado** | Rota criada com ISR (`revalidate = 60`) na Phase 4; conteúdo completo pendente. |
...
| Testes (Vitest + Playwright) | **Funcional (Vitest)** | 229 testes passando em 17 arquivos (action-utils, notifications, security/rbac-regression, security/rbac-scope-verification, security/rbac-scope-leak, security/xss-payload-validation, performance/composite-index-presence, critical/acceptance-critical-flows). Playwright E2E a configurar. |
