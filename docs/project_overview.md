...
| Admin — Gestão de Equipe | **Parcial** | Lista de consultoras (`/admin/consultoras`) com métricas agregadas; perfil detalhado de revendedora (`/admin/revendedoras/[id]`) com maletas, pontos, dados bancários, documentos; perfil detalhado de consultora (`/admin/consultoras/[id]`) com grupo e comissões. **Sidebar da consultora implementada**: layout e bottom nav filtrados por role, rotas `/admin/minha-conta` e `/admin/minha-conta/comissoes` com resumo e extrato. Criação via Supabase Auth + convite por email ainda pendente. |
