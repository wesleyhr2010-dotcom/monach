...
| Admin — Gestão de Equipe | **Parcial** | Lista de consultoras (`/admin/consultoras`) com métricas agregadas; perfil detalhado de revendedora (`/admin/revendedoras/[id]`) com maletas, pontos, dados bancários, documentos; **edição completa da revendedora** (`/admin/revendedoras/[id]/editar`) com campos de identidade, endereço, candidatura e conta (taxa de comissão, consultora, status ativo/inativo) via `atualizarRevendedoraAdmin`; perfil detalhado de consultora (`/admin/consultoras/[id]`) com grupo e comissões. **Sidebar da consultora implementada**: layout e bottom nav filtrados por role, rotas `/admin/minha-conta` e `/admin/minha-conta/comissoes` com resumo e extrato. **Criação via Supabase Auth + convite por email implementada**: `criarColaboradora` e `criarRevendedora` criam usuário no Auth, gravam `auth_user_id` e enviam convite via Brevo. |
...
| Admin — Brindes | **Funcional** | CRUD completo (`/admin/brindes`, `/admin/brindes/nuevo`, `/admin/brindes/[id]/editar`), gestão de solicitações (`/admin/brindes/solicitudes`), fluxo separar→entregar com notificação push, cancelamento com reembolso de pontos. **Upload de imagem via R2 implementado (2026-05-16):** componente `BrindeImageUploader` com drag-and-drop, upload imediato a `brindes/` no R2, preview e fallback URL manual. Ref.: `docs/admin/SPEC_ADMIN_BRINDES.md`. |
| Portal Revendedora (PWA) | **Funcional** | Login (com recuperação de senha via email + redefinição), onboarding completo (incluindo aceite de contrato), perfil (resumo/datos/bancario/soporte/notificações), maleta (listagem/detalhes/venta/devolução), catálogo (busca/filtro/compartilhamento), progresso/brindes (atalhos "Extracto →" e "Canjear Regalos →" no header de `/app/progreso`; aba "Más" ativa em rotas `/app/progreso*`), centro de notificações persistente, menu "Más". **Dashboard de desempenho (`/app/desempeno`) implementado com métricas, filtro de período, gráfico recharts e top produtos.** View Transitions funcionando. |
...
| Gamificação (motor) | **Funcional com brechas de segurança** | `awardPoints` respeita tipo (único/diário/mensal/evento), limite diário e flag ativo. 7 regras seedadas. Tela `/app/progresso` mostra tarefas e progresso. Admin pode configurar regras e níveis. **Brechas identificadas (2026-05-06):** `awardPoints` exportada publicamente sem validação de ownership; `registrarPuntosCompartirCatalogo` não valida ação real; sem rate limiting nas actions de pontos; sem audit trail (`awarded_by`, `ip_address`); sem testes de segurança de gamificação. Ref.: `SPEC_GAMIFICACAO_OVERVIEW.md`, `next_steps.md` §"Segurança da Gamificação". |
...
| Dark Mode & Temas | **Concluído** | Phase 19 (2026-05-15): tokens CSS com variantes dark/light. Phase 20 (2026-05-15): migração de todas as cores hex hardcoded no PWA para tokens. Phase 21 (2026-05-16): `AppThemeProvider` e `AdminThemeProvider` escopados, anti-flash script, `SonnerThemer` seguindo `resolvedTheme`. Phase 22 (2026-05-16): `ThemeToggle` componente reutilizável (variantes `app`/`admin`) com toggle binário sol/lua em `/app/perfil` e `/admin/minha-conta`. Preferência persistida em localStorage. Build e lint passam. Ref.: `ROADMAP.md` Phases 19–22. |
