# SPEC — Catálogo e Compartilhamento de Fotos

## Objetivo
Exibir à revendedora uma vitrine dos produtos disponíveis na sua maleta ativa e permitir que compartilhe fotos selecionadas via Web Share API com clientes.

## Atores
- **Revendedora** — navega, filtra e compartilha itens.
- **Cliente final** — recebe as imagens/links via WhatsApp ou outro app.
- **Cloudflare R2** — hospedagem das imagens dos produtos.

## Fluxo
1. Revendedora acessa `/app/catalogo` e vê apenas os itens da maleta `ativa` com saldo (`quantidade_vendida < quantidade_enviada`).
2. Filtra por categoria ou busca por nome; pode compartilhar item individual (link via Web Share).
3. Para compartilhar múltiplos, toca em "Compartir" e vai para `/app/catalogo/compartir`.
4. Seleciona até 10 imagens (multi-select com checkmark verde).
5. Toca em "📲 Compartir" — sistema baixa blobs do R2, gera `File[]` e invoca `navigator.share({ files })`.
6. Cliente final recebe as imagens no app escolhido.

## Regras de negócio
- Mostra somente itens da maleta **ativa** da revendedora logada, nunca catálogo geral.
- Seleção múltipla limitada a **10 imagens**.
- Barra inferior de ação é sticky, fica **acima** da `BottomNavigation` sem ocultá-la.
- Navegação global permanece visível em todo o fluxo.
- Imagens selecionadas têm borda primária `#35605a` e checkmark verde.

## Edge cases
- Maleta sem itens disponíveis → catálogo vazio com empty state.
- Dispositivo sem suporte a `navigator.share({ files })` → fallback para compartilhar links/texto.
- Falha ao baixar imagem do R2 → erro amigável, permite retry.
- Tentativa de selecionar mais de 10 → bloqueia e mostra aviso.
- Revendedora sem maleta ativa → tela vazia com CTA para solicitar maleta.

## Dependências
- `SPEC_MALETA.md` — modelo `maleta` + `maleta_item`.
- `SPEC_API_UPLOAD_R2.md` — URLs públicas do R2.
- `SPEC_ADMIN_PRODUTOS.md` — `product_variant.image_url`.
