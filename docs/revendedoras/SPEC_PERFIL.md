# SPEC — Perfil da Revendedora (Ver, Editar, Documentos, Bancário)

## Objetivo
Disponibilizar à revendedora um hub de perfil onde pode consultar seus dados, editar informações pessoais, gerenciar documentos/contratos e configurar dados bancários para pagamento de comissões.

## Atores
- **Revendedora** — edita seu próprio perfil.
- **Admin** — visualiza e valida documentos (`SPEC_ADMIN_DOCUMENTOS_ACERTOS.md`), define `taxa_comissao`.
- **Consultora** — relação exibida no perfil (read-only).

## Fluxo
1. Revendedora acessa `/app/perfil` e vê avatar, nome, rank, tasa de comisión, consultora vinculada e menu de submenus.
2. "Mis Datos" → `/app/perfil/datos` edita whatsapp, avatar, etc.
3. "Documentos y Contratos" → `/app/perfil/documentos` envia/consulta documentos (`ResellerDocumento`) e contratos.
4. "Datos Bancarios" → `/app/perfil/bancario` cria/edita `DadosBancarios`.
5. "Notificaciones" → ver `SPEC_NOTIFICACOES.md`.
6. "Soporte y Ayuda" → abre canal de suporte (WhatsApp).
7. "Cerrar Sesión" → logout Supabase.

## Regras de negócio
- `taxa_comissao` é **read-only** para revendedora — apenas admin altera.
- Correo exibido vem de `supabase.auth.user.email`, não de `Reseller`.
- Dados bancários pertencem ao portal da **revendedora** (decisão firmada); prototipo antigo da colaboradora foi descartado.
- Rank exibido usa `getRankAtual` com pontos históricos.
- Campos obrigatórios ao salvar dados: nome, whatsapp.
- Upload de documentos via R2 (ver `SPEC_API_UPLOAD_R2.md`).

## Edge cases
- Revendedora sem consultora vinculada → exibe "Sin consultora asignada".
- Documento em revisão pelo admin → mostra status `pendente`.
- Documento rejeitado → notificação + motivo visível; permite reenvio.
- Dados bancários incompletos → admin não consegue processar comissão.
- Logout quebra OneSignal login → chamar `OneSignal.logout()` antes de redirecionar.

## Dependências
- `SPEC_LOGIN.md` — autenticação + logout.
- `SPEC_ADMIN_DOCUMENTOS_ACERTOS.md` — revisão de documentos pelo admin.
- `SPEC_API_UPLOAD_R2.md` — upload de arquivos.
- `SPEC_NOTIFICACOES.md` — preferências.
- `SPEC_DATABASE.md` — `Reseller`, `ResellerDocumento`, `Contrato`, `DadosBancarios`.
